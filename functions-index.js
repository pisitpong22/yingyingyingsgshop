...
exports.createPaymentIntent = onCall(
  ...
  async (request) => {
    ...
    for (const it of items) {
      ...
      // Unified product schema (it.type === "products") uses different
      // fields than the legacy amulets/accessories shape — check those too.
      if (it.type === "products") {
        if (canonical.allowCheckout === false || canonical.allowEnquiryOnly === true) {
          throw new HttpsError(
            "failed-precondition",
            `Item is not available for checkout: ${canonical.name || it.id}`
          );
        }
        if (canonical.stockStatus === "sold_out") {
          throw new HttpsError(
            "failed-precondition",
            `Item already sold out: ${canonical.name || it.id}`
          );
        }
      }
      const price = Number(
        (canonical.salePrice != null && canonical.salePrice !== "" && Number(canonical.salePrice) > 0)
          ? canonical.salePrice
          : (canonical.price || 0)
      );
      total += price;
